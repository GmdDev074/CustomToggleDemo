...
class DeveloperActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setupToggle(R.id.toggle_debug, R.id.text_debug,
            "Debug Mode is ON", "Debug Mode is OFF")

        setupToggle(R.id.toggle_logs, R.id.text_logs,
            "Logs are Enabled", "Logs are Disabled")

        setupToggle(R.id.toggle_mock, R.id.text_mock,
            "Mock Locations are Enabled", "Mock Locations are OFF")

        setupToggle(R.id.toggle_bg_tasks, R.id.text_bg_tasks,
            "Background Tasks are Enabled", "Background Tasks are Disabled")

        setupToggle(R.id.toggle_battery, R.id.text_battery,
            "Battery Saver is ON", "Battery Saver is OFF")

        setupToggle(R.id.toggle_strict, R.id.text_strict,
            "Strict Mode is ON", "Strict Mode is OFF")

        setupToggle(R.id.toggle_bounds, R.id.text_bounds,
            "Layout Bounds are Visible", "Layout Bounds are Hidden")

        setupToggle(R.id.toggle_awake, R.id.text_awake,
            "Keep Screen Awake is ON", "Keep Screen Awake is OFF")

        setupToggle(R.id.toggle_gpu, R.id.text_gpu,
